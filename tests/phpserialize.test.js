...
describe('fn: phpUnserialize', () => {
    test( 'fn: phpUnserialize( undefined )'       , () => { expect( Ofn.phpUnserialize() ).toBe( undefined ); } );
    test( 'fn: phpUnserialize( str )'             , () => { expect( Ofn.phpUnserialize( 'chacho' ) ).toBe( 'chacho' ); } );
    test( 'fn: phpUnserialize( str php )'         , () => { expect( Ofn.phpUnserialize( 'a:1:{s:6:\"chacho\";b:1;}' ) ).toEqual( { chacho: true } ); } );
    test( 'fn: phpUnserialize( str json )'        , () => { expect( Ofn.phpUnserialize( '{"chacho":true}' ) ).toEqual( { chacho: true } ); } );
    test( 'fn: phpUnserialize( str json, strict )', () => { expect( Ofn.phpUnserialize( '{"chacho":true}', true ) ).toBe( '{"chacho":true}' ); } );
});
