...
describe( 'fn: objIsEmpty', () => {
     ...
     test( 'fn: objIsEmpty( obj )', () => {
         ...
         expect( Ofn.objIsEmpty<Custom>( { chacho: true } ) ).toBe( false );
         expect( Ofn.objIsEmpty<Custom>( {} ) ).toBe( true );
     } );
     test( 'fn: objIsEmpty( obj )', () => {
...
describe( 'fn: objGetMethods', () => {
    test( 'fn: objGetMethods( undefined )', () => {
        expect( Ofn.objGetMethods( undefined ) ).toEqual( [] );
    } );
...
describe( 'fn: objHasKeys', () => {
    ...
    //     expect( Ofn.objHasKeys( undefined ) ).toBe( false );
    // } );
    ...
    test( 'fn: objHasKeys( obj, arr )', () => {
        interface Custom { chacho: boolean, loco?: boolean }
        const obj: Custom = { chacho: true };
        expect( Ofn.objHasKeys<Custom, keyof Custom>( obj, [ 'chacho', 'loco' ] ) ).toBe( false );
    } );
    ...
    test( 'fn: objHasKeys( arr, arr )', () => {
        expect( Ofn.objHasKeys<number[], number>( [ 7, 8, 9 ], [ 1, 2 ] ) ).toBe( true );
    } );
} );

describe( 'fn: getObjectMissedKeys', () => {
    // test( 'fn: getObjectMissedKeys( undefined )', () => {
    //     expect( Ofn.getObjectMissedKeys( undefined ) ).toEqual( [] );
    // } );
    ...
    test( 'fn: getObjectMissedKeys( obj, arr )', () => {
        interface Custom { chacho: boolean, loco?: boolean }
        expect( Ofn.getObjectMissedKeys<Custom, keyof Custom>( { chacho: true }, [ 'chacho', 'loco' ] ) ).toEqual( [ 'loco' ] );
    } );
    ...
    test( 'fn: getObjectMissedKeys( arr, arr )', () => {
        expect( Ofn.getObjectMissedKeys( [ 7, 8, 9 ], [ 3, 2, 1, '4' ] ) ).toEqual( [ 3, '4' ] );
    } );
} );

describe( 'fn: cloneObject', () => {
    test( 'fn: cloneObject( undefined )', () => {
        expect( Ofn.cloneObject( undefined ) ).toEqual( {} );
    } );
    ...
    test( 'fn: cloneObject( obj )', () => {
        interface Custom { chacho: boolean, loco: boolean}
        let obj1 = { chacho: true, loco: false };
        let obj2 = Ofn.cloneObject<Custom>( obj1 );
        expect( obj1 ).toEqual( obj2 );
...
describe( 'fn: cloneObjectWithKeys', () => {
    ...
    //     expect( Ofn.cloneObjectWithKeys( undefined ) ).toEqual( {} );
    // } );
    ...
    test( 'fn: cloneObjectWithKeys( obj )', () => {
        interface Custom { chacho: boolean, loco: boolean }
        let obj1: Custom = { chacho: true, loco: false };
        let obj2 = Ofn.cloneObjectWithKeys<Custom>( obj1, [ 'loco' ] );
        expect( Object.keys( obj2 ) ).toEqual( [ 'loco' ] );
    ...
    test( 'fn: cloneObjectWithKeys( arr )', () => {
        interface Custom1 { chacho: boolean }
        interface Custom2 { loco: boolean }
        type Customs = [Custom1, Custom2];
        let obj1: Customs = [ { chacho: true }, { loco: false } ];
        let obj2 = Ofn.cloneObjectWithKeys<Customs, number>( obj1, [ 1 ] ) as Custom2[];
        expect( Object.keys( obj2 ) ).toEqual( [ '1' ] );
...
describe( 'fn: cloneObjectWithoutKeys', () => {
    ...
    //     expect( Ofn.cloneObjectWithoutKeys( undefined ) ).toEqual( {} );
    // } );
    ...
    test( 'fn: cloneObjectWithoutKeys( obj )', () => {
        ...
        type OmitKeys = 'loco';
        let obj1 = { chacho: true, loco: false };
        let obj2 = Ofn.cloneObjectWithoutKeys<Custom, OmitKeys>( obj1, [ 'loco' ] );
        expect( Object.keys( obj2 ) ).toEqual( [ 'chacho' ] );
    ...
    test( 'fn: cloneObjectWithoutKeys( arr )', () => {
        ...
        type Customs = [Custom1, Custom2];
        let obj1: Customs = [ { chacho: true }, { loco: false } ];
        let obj2 = Ofn.cloneObjectWithoutKeys<Customs, number>( obj1, [ 1 ] ) as Custom1[];
        expect( Object.keys( obj2 ) ).toEqual( [ '0' ] );
...
describe( 'fn: mergeObjectsDeep', () => {
    test( 'fn: mergeObjectsDeep( undefined )', () => {
        expect( Ofn.mergeObjectsDeep( undefined ) ).toEqual( {} );
    } );
    ...
    test( 'fn: mergeObjectsDeep( obj1, obj2 ) no deep', () => {
        interface Custom { chacho: boolean, loco: boolean, tio: boolean }
        let obj1: Partial<Custom> = { chacho: true, loco: false };
        let obj2: Partial<Custom> = { loco: true, tio: true };
        let objMerge = Ofn.mergeObjectsDeep<Custom>( obj1, obj2 );
        expect( objMerge ).toEqual( { chacho: true, loco: true, tio: true } );
    } );
    ...
    test( 'fn: mergeObjectsDeep( obj1, obj2, obj3, obj4 ) deep', () => {
        ...
        expect( Ofn.mergeObjectsDeep( obj1, obj2, obj3, obj4 ) )
            .toEqual( { chacho: true, loco: { fleje: false }, tio: false } );
    } );
} );

describe( 'fn: objToStringSqlSet', () => {
    test( 'fn: objToStringSqlSet( undefined )', () => {
        expect( Ofn.objToStringSqlSet( undefined ) ).toBe( '' );
    } );
    test( 'fn: objToStringSqlSet( str )', () => {
        expect( Ofn.objToStringSqlSet( 'chacho' ) ).toBe( '' );
    } );
    test( 'fn: objToStringSqlSet( obj )', () => {
        expect( Ofn.objToStringSqlSet( { id: 20, name: "'oro'", today: 'NOW()' } ) )
            .toBe( " id = 20, name = 'oro', today = NOW() " );
    } );
    test( 'fn: objToStringSqlSet( obj )', () => {
        interface Custom { id: number, name: string, today?: string }
        expect( Ofn.objToStringSqlSet<Custom>( { id: 20, name: "'oro'", today: 'NOW()' } ) )
            .toBe( " id = 20, name = 'oro', today = NOW() " );
    } );
} );
...
describe( 'fn: objToStringAttr', () => {
    test( 'fn: objToStringAttr( undefined )', () => {
        expect( Ofn.objToStringAttr( undefined ) ).toBe( '' );
    } );
    ...
    test( 'fn: objToStringAttr( obj )', () => {
        interface Custom { id: number, name: string, today?: string }
        expect( Ofn.objToStringAttr<Custom>( { id: 20, name: 'oro' } ) ).toBe( ' id="20" name="oro" ' );
    } );
    test( 'fn: objToStringAttr( obj w html )', () => {
        expect( Ofn.objToStringAttr( { id: 20, name: '<span>oro</span>' } ) )
            .toBe( ' id="20" name="&lt;span&gt;oro&lt;/span&gt;" ' );
    } );
} );

describe( 'fn: objToStringAttrData', () => {
    test( 'fn: objToStringAttrData( undefined )', () => {
        expect( Ofn.objToStringAttrData( undefined ) ).toBe( '' );
    } );
    ...
    test( 'fn: objToStringAttrData( obj )', () => {
        interface Custom { id: number, name: string, today?: string }
        expect( Ofn.objToStringAttrData<Custom>( { id: 20, name: 'oro' } ) )
            .toBe( ' data-id="20" data-name="oro" ' );
    } );
    test( 'fn: objToStringAttrData( obj w html )', () => {
        expect( Ofn.objToStringAttrData( { id: 20, name: '<span>oro</span>' } ) )
...
describe( 'fn: objToStringAttrJs', () => {
    test( 'fn: objToStringAttrJs( undefined )', () => {
        expect( Ofn.objToStringAttrJs( undefined ) ).toBe( '' );
    } );
    ...
    test( 'fn: objToStringAttrJs( obj )', () => {
        interface Custom { id: number, name: string, today?: string }
        expect( Ofn.objToStringAttrJs<Custom>( { id: 20, name: 'oro' } ) ).toBe( ' id:20; name:oro ' );
    } );
} );
