...
describe( 'fn: arrayValuesByKey', () => {
    ...
    test( 'fn: arrayValuesByKey( arr, key, T, V )', () => {
        interface Custom { label: string, name: string }
        type Label = 'chacho' | 'loco';
        let arr: Custom[] = [ { label: 'chacho', name: 'chacho' }, { label: 'loco', name: 'loco' } ];
        let obj = Ofn.arrayValuesByKey<Custom, Label>( arr, 'label' );
        expect( obj ).toEqual( [ 'chacho', 'loco' ] );
    } );
...
describe( 'fn: arrayGroupByKey', () => {
    ...
    test( 'fn: arrayGroupByKey( arr, key missed, strict, T )', () => {
        interface Custom { label: string, chacho: string }
        let arr: Custom[] = [
            { label: 'chacho', chacho: 'chacho' },
...
describe( 'fn: arrayGetUnique', () => {
    ...
    test( 'fn: arrayGetUnique( arr w obj 2 )', () => {
        interface Custom { chacho: number }
        let arr: Custom[] = [
            { chacho: 2 },
            { chacho: 4 },
            { chacho: 3 },
