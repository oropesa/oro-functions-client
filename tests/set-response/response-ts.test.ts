...
describe('fn: setResponseOK', () => {
  ...
  test('fn: setResponseOK( obj2 )', () => {
    expect(Ofn.setResponseOK({ status: 'tio', chacho: true })).toEqual({
      status: true,
      chacho: true,
    });
  });
  ...
  test('fn: setResponseOK( obj3 )', () => {
    expect(Ofn.setResponseOK({ data: { status: 'tio', chacho: true } })).toEqual({
      status: true,
      data: { status: 'tio', chacho: true },
    });
  });
...
describe('fn: setResponseKO', () => {
  ...
  test('fn: setResponseKO( str )', () => {
    expect(Ofn.setResponseKO('chacho')).toEqual({ status: false, error: { msg: 'chacho' } });
  });
  test('fn: setResponseKO( obj )', () => {
    expect(Ofn.setResponseKO({ chacho: true })).toEqual({ status: false, error: { chacho: true } });
  });
  test('fn: setResponseKO( obj2 )', () => {
    expect(Ofn.setResponseKO({ status: 'tio', chacho: true })).toEqual({
      status: false,
      error: { status: 'tio', chacho: true },
    });
  });
  ...
  test('fn: setResponseKO( str, obj.msg )', () => {
    expect(Ofn.setResponseKO('chacho', { msg: 'tio' })).toEqual({
      status: false,
