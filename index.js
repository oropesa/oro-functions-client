...
class Ofn {
    ...
    static arraySortByKey( arr, key, sortArray ) {
        if( ! Ofn.isArray( arr ) ) { return []; }
        Ofn.isNumber( key ) && ( key += '' );
        if( ! Ofn.isString( key ) ) { return []; }

        let values;
        if( Ofn.isArray( sortArray ) ) {
    ...
    static dateObjByToday( opts = {} ) {
        Ofn.isString( opts ) && ( opts = { sep: opts } );
        ! Ofn.isObject( opts ) && ( opts = { sep: '/' } );
        ! Ofn.isString( opts.sep ) && ( opts.sep = '/' );
        return Ofn.dateObjByDate( (new Date()), opts );
    }

    static dateObjByTimestamp( timestamp, opts = {} ) {
        if( ! Ofn.isNumber( timestamp ) ) { return null; }

        Ofn.isString( opts ) && ( opts = { sep: opts } );
        ! Ofn.isObject( opts ) && ( opts = { sep: '/' } );
        ! Ofn.isString( opts.sep ) && ( opts.sep = '/' );
        return Ofn.dateObjByDate( (new Date( timestamp )), opts );
    }

    static dateObjBySql( sqldate, opts = {} ) {
        ...
        ! Ofn.isObject( opts ) && ( opts = { sep: '/' } );
        ! Ofn.isString( opts.sep ) && ( opts.sep = '/' );
        return Ofn.dateObjByDate( (new Date( sqldate )), opts );
    }

    static dateObjByHtml( html, opts = {} ) {
        ...
        if( ! REGEXP.DATETIME_FN( opts.sep ).test( html ) ) { return null; }
    ...
    static dateObjByDate( date, opts = {} ) {
        if( Ofn.type( date ) !== 'date' ) { return null; }
        ...
        if( Ofn.isString( opts.timezone ) && ! isTimezoneAllowed( opts.timezone ) ) { return null; }

        dateObj.timezone = opts.timezone ? opts.timezone : Intl.DateTimeFormat().resolvedOptions().timeZone
        dateObj.sep = opts.sep;

        let millis = Ofn.strPad( date.getMilliseconds(), 3, '0', 'right' );
        const dateArr = date.toLocaleString( 'en-GB', { timeZone: dateObj.timezone } ).split( /[^0-9]/ );
        // dateArr = [ day, month, year, empty, hour, minute, second ]

        dateObj.local = `${dateArr[ 2 ]}-${dateArr[ 1 ]}-${dateArr[ 0 ]}T${dateArr[ 4 ]}:${dateArr[ 5 ]}:${dateArr[ 6 ]}.${millis}Z`;

        dateObj.year = dateArr[ 2 ];
        dateObj.month = dateArr[ 1 ];
        dateObj.day = dateArr[ 0 ];
        dateObj.hour = dateArr[ 4 ];
        dateObj.minute = dateArr[ 5 ];
        dateObj.second = dateArr[ 6 ];
...
function isTimezoneAllowed( timezone ) {
    if( ! Ofn.isString( timezone ) ) { return false; }
